...
# 모든 조합 순회
for wbs, tr, val, num in product(WBS_LIST, TR_LIST,VAL_LIST, NUM_LIST):
    # 로그 파일 경로
    path = f"./experiments/second_2/logs/user_log_wbs{wbs}_tr{tr}_val{val}_num{num}.log"
...
# 출력 또는 저장
print(df.head())
df.to_csv("rocksdb_benchmark_summary_en(userlog).csv", index=False)